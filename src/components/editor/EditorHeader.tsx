...
import { Layers, ChevronLeft, ChevronDown, FileDown, LayoutPanelTop, Check, Loader2 } from 'lucide-react';
...
export const EditorHeader = ({ showLayers, setShowLayers, onExport, onSaveTemplate, isSavingTemplate, resumeName }: EditorHeaderProps) => {
        ...
        <header className="h-14 bg-white border-b border-gray-200 flex items-center px-4 justify-between shrink-0 z-20 sticky top-0">
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                    <h1 className="text-sm font-bold text-gray-900 leading-none">{resumeName || 'Untitled Design'}</h1>
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-1">A4 Standard • 210 x 297 mm</span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    className={`p-2 rounded-xl transition-all duration-300 ${showLayers ? 'bg-purple-50 text-purple-600 ring-1 ring-purple-100' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
                    title="Toggle Layers"
                >
                    <Layers size={20} />
                </button>
                ...
                <div className="relative" ref={menuRef}>
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-xl text-sm font-bold transition-all shadow-md active:scale-95"
                    >
                        Share
